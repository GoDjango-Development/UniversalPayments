...
from applications.forms import StripeApplicationForm
from applications.models import StripeApplication


class StripeApplicationAdmin(admin.ModelAdmin):
    list_display_links = None
    list_per_page = 20
    form = StripeApplicationForm
    ...
    def get_queryset(self, request):
        if request.user.is_superuser:
            ...
            return StripeApplication.objects.all()
        else:
            # Return only objects created by the current user
            return StripeApplication.objects.filter(user=request.user)
    ...
    def title_column(self, obj):
        return mark_safe('<a href="%s" style="cursor: pointer;" data-toggle="tooltip" title="%s"><b>%s</b></a>' % (
            reverse_lazy('object_details', kwargs={'model_name': 'stripeapplication', 'pk': obj.pk}),
            "Detalles",
            obj.name
        ))
    ...
    def action_column(self, obj):
        html = '<a href="%s" style="cursor: pointer; margin-right: 10px;" data-toggle="tooltip" title="%s">' \
               ...
               '</a>' % (
                   '/admin/applications/stripeapplication/%i' % (obj.pk,),
                   "Editar"
               )
        html += '<a href="%s" style="cursor: pointer;" data-toggle="tooltip" class="btn-delete" title="%s">' \
                ...
                '</a>' % (
                    '/admin/applications/stripeapplication/%i/delete/' % (obj.pk,),
                    "Eliminar"
                )
...
admin.site.register(StripeApplication, StripeApplicationAdmin)