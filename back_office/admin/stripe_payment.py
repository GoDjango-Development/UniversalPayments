...
from payments.forms import StripePaymentForm
...
class StripePaymentAdmin(admin.ModelAdmin):
    ...
    list_display = ('title_column', 'stripe_app', 'get_datetime', 'get_status',)
    search_fields = ('stripe_app',)
    
    def get_queryset(self, request):
        if request.user.is_superuser:
            # Return all objects for superusers
            return StripePayment.objects.all()
        else:
            # Return only objects created by the current user
            return StripePayment.objects.filter(user=request.user)
    ...
    def title_column(self, obj):
        return mark_safe('<a href="%s" style="cursor: pointer;" data-toggle="tooltip" title="%s"><b>%s</b></a>' % (
            reverse_lazy('object_details', kwargs={'model_name': 'stripepayment', 'pk': obj.pk}),
            "Detalles",
