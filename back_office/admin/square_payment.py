...
from payments.forms import SquarePaymentForm
...
class SquarePaymentAdmin(admin.ModelAdmin):
    ...
    list_display = ('title_column', 'square_app', 'get_datetime', 'get_status',)
    search_fields = ('square_app',)
    
    def get_queryset(self, request):
        if request.user.is_superuser:
            # Return all objects for superusers
            return SquaresPayment.objects.all()
        else:
            # Return only objects created by the current user
            return SquaresPayment.objects.filter(user=request.user)
    ...
    def title_column(self, obj):
        return mark_safe('<a href="%s" style="cursor: pointer;" data-toggle="tooltip" title="%s"><b>%s</b></a>' % (
            reverse_lazy('object_details', kwargs={'model_name': 'squarespayment', 'pk': obj.pk}),
            "Detalles",
            obj.idempotency_key
        ))
...
admin.site.register(SquaresPayment, SquarePaymentAdmin)